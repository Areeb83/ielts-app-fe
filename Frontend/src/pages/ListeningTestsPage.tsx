...
export function ListeningTestsPage() {
  ...
  const handleReview = async (test: any) => {
    ...
    if (!attemptId) {
      navigate(`/${examType}/listening/${test.id}/result`, {
        state: { testTitle: `C${test.bookNumber} Listening Test ${test.testNumber}`, testType: 'listening', totalQuestions: 40, timeSpent: '--:--', userAnswers: {}, bandScore: test.result?.bandScore, correctAnswers: test.result?.correctAnswers },
      });
      return;
    }
    ...
    try {
      ...
      navigate(`/${examType}/listening/${test.id}/result`, {
        state: {
          testTitle: `C${test.bookNumber} Listening Test ${test.testNumber}`,
          testType: 'listening',
          totalQuestions: 40,
                            ...
                            >
                              <div className="text-left">
                                ...
                                <div className="text-xs text-gray-400 mt-1">
                                  {isCompleted ? `${result.correctAnswers}/${result.totalQuestions} correct` : '\u00A0'}
                                </div>
                              </div>
                                  ...
                                  <>
                                    ...
                                    {isCompleted && (
                                      <button
                                        ...
                                        className="w-full px-4 py-2 rounded-lg font-medium transition-colors border-2 border-gray-200 text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                                      >
                                        {reviewingId === test.id ? 'Loading...' : 'Review'}
                                      </button>
                                    )}
                                  </>
                                )}
