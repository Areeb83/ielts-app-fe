...
import { useWritingBooks } from '../hooks/useWritingTests';
...
export function WritingTestsPage() {
  ...
  const { books: currentBooks, stats, pagination, loading, currentPage, changePage } = useWritingBooks(
        ...
        <div className="page-container">
          <div className="space-y-10">
                ...
                <motion.div key={book.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.05 }} className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden p-6">
                  <div className="flex items-start gap-6 mb-6">
                    ...
                    <div className="text-left">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600">
                        <span className="flex items-center gap-1"><BookOpen className="w-4 h-4 text-purple-800" />{book.totalTests} Tests</span>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    {book.tests.map((test) => {
                      ...
                      const isPending = isCompleted && (test.result as any)?.pending;
                      const percent = isCompleted && bandScore != null && bandScore > 0 ? Math.max(0, Math.min(100, Math.round((bandScore / 9) * 100))) : 0;
                      const r = 45;
                      ...
                      return (
                        <div key={test.id} className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm hover:shadow-md transition-all p-4">
                          <div className="text-left"><div className="font-semibold text-gray-900">Writing Test {test.testNumber}</div></div>
                                ...
                                {isCompleted ? (
                                  isPending ? (
                                    <>
                                      ...
                                      <div className="text-xs text-gray-500 mt-0.5">Review</div>
                                    </>
                                  ) : (
                                    <div className="text-lg font-bold text-gray-900">{bandScore ?? 0}</div>
                                  )
                                ) : (
                          ...
                          <div className="mt-5 flex flex-col gap-2">
                            <button type="button" onClick={() => guardedNavigate(`/${examType}/writing/${test.id}`)} className={`w-full px-4 py-2 rounded-lg font-medium transition-colors border-2 ${isCompleted ? 'bg-white text-purple-800 border-purple-700 hover:bg-purple-50' : 'bg-purple-800 text-white border-purple-800 hover:bg-purple-900'}`}>
            ...
            <div className="flex justify-center items-center gap-4 mt-12">
              <button onClick={() => changePage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className={`p-2 rounded-lg border-2 transition-all ${currentPage === 1 ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-purple-100 text-purple-800 hover:bg-purple-50 hover:border-purple-700'}`}>
                <ChevronLeft className="w-6 h-6" />
              </button>
              <div className="flex gap-2">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i + 1} onClick={() => changePage(i + 1)} className={`w-12 h-12 rounded-xl font-bold transition-all border-2 ${currentPage === i + 1 ? 'bg-purple-800 text-white border-purple-800 shadow-md scale-110' : 'bg-white text-gray-600 border-gray-100 hover:border-purple-700 hover:text-purple-800'}`}>
                    {i + 1}
                  </button>
                ))}
              </div>
              <button onClick={() => changePage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className={`p-2 rounded-lg border-2 transition-all ${currentPage === totalPages ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-purple-100 text-purple-800 hover:bg-purple-50 hover:border-purple-700'}`}>
