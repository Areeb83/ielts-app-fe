import { BookText, Clock, BookOpen, ChevronLeft, ChevronRight, GraduationCap, Globe } from 'lucide-react';
        ...
        <div className="page-container">
          <div className="space-y-10">
                  ...
                  <div className="flex items-start gap-6 mb-6">
                    ...
                    <div className="text-left">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-600">
                        <span className="flex items-center gap-1"><BookOpen className="w-4 h-4 text-green-800" />4 Tests</span>
                    ...
                    {book.tests.map((test) => {
                      ...
                      return (
                        <div key={test} className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm hover:shadow-md transition-all p-4">
                          <div className="text-left"><div className="font-semibold text-gray-900">Reading Test {test}</div></div>
                          ...
                          <div className="mt-5 flex flex-col gap-2">
                            <button type="button" className={`w-full px-4 py-2 rounded-lg font-medium transition-colors border-2 ${isCompleted ? 'bg-white text-green-800 border-green-700 hover:bg-green-50' : 'bg-green-800 text-white border-green-800 hover:bg-green-900'}`}>
            ...
            <div className="flex justify-center items-center gap-4 mt-12">
              <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className={`p-2 rounded-lg border-2 transition-all ${currentPage === 1 ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-green-100 text-green-800 hover:bg-green-50 hover:border-green-700'}`}>
                <ChevronLeft className="w-6 h-6" />
              </button>
              <div className="flex gap-2">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`w-12 h-12 rounded-xl font-bold transition-all border-2 ${currentPage === i + 1 ? 'bg-green-800 text-white border-green-800 shadow-md scale-110' : 'bg-white text-gray-600 border-gray-100 hover:border-green-700 hover:text-green-800'}`}>
                    {i + 1}
                  </button>
                ))}
              </div>
              <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className={`p-2 rounded-lg border-2 transition-all ${currentPage === totalPages ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-green-100 text-green-800 hover:bg-green-50 hover:border-green-700'}`}>
